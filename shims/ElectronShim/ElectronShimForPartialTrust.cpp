#include "ElectronShimForPartialTrustImpls.h"
...
///
// getLocalPipeName
// Given an input string, prefixes the pipe path with a UWP accepted pipe path
    ...
    _In_opt_ HANDLE   hTemplateFile) noexcept
{
    ...
    // If we have already shimmed this function, fall back on the native call
...
template <typename CharT>
BOOL WINAPI CreateDirectoryShim(
...
DECLARE_STRING_SHIM(impl::CopyFile, CopyFileShim);

template <typename CharT>
BOOL WINAPI DeleteFileShim(
    _In_ const CharT* lpFileName)
{
    auto guard = g_reentrancyGuard.enter();
    if (guard) 
...
template <typename CharT>
HANDLE WINAPI FindFirstFileExShim(
    _In_ const CharT*       lpFileName,
    ...
    _In_ DWORD              dwAdditionalFlags)
{
    ...
    if (guard) 
    {
        ...
        {
            findData = shims::is_ansi<CharT> ? &wideData : reinterpret_cast<WIN32_FIND_DATAW*>(lpFindFileData);
        }
        ...
        if ((lpFindFileData != nullptr) && (shims::is_ansi<CharT>) && status)
        {
...
DECLARE_STRING_SHIM(impl::FindFirstFileEx, FindFirstFileExShim);

template <typename CharT>
DWORD WINAPI GetFileAttributesShim(
    _In_ const CharT* lpFileName)
{
    auto guard = g_reentrancyGuard.enter();
    if (guard) 
...
template <typename CharT>
BOOL WINAPI GetFileAttributesExShim(
    _In_ const CharT*           lpFileName,
...
template <typename CharT>
BOOL WINAPI RemoveDirectoryShim(
    _In_ const CharT* lpPathName)
{
    auto guard = g_reentrancyGuard.enter();
    if (guard) 
...
template <typename CharT>
BOOL WINAPI SetFileAttributesShim(
    _In_ const CharT* lpFileName,
...
template <typename CharT>
HANDLE WINAPI CreateNamedPipeShim(
...
DECLARE_STRING_SHIM(impl::CreateNamedPipe, CreateNamedPipeShim);

template <typename CharT>
BOOL WINAPI CallNamedPipeShim(
...
DECLARE_STRING_SHIM(impl::CallNamedPipe, CallNamedPipeShim);

template <typename CharT>
BOOL WINAPI WaitNamedPipeShim(
    _In_ const CharT* lpNamedPipeName,
    _In_ DWORD        nTimeOut)
{
    return impl::WaitNamedPipe(getLocalPipeName(lpNamedPipeName).c_str(), nTimeOut);
}
DECLARE_STRING_SHIM(impl::WaitNamedPipe, WaitNamedPipeShim);
...
static bool detectPipe(std::wstring_view pipeName)
{
    ...
    auto pathType = shims::path_type(pipeName.data());
    if ((pathType != shims::dos_path_type::local_device) &&
        (pathType != shims::dos_path_type::root_local_device))
    {
        return false;
    }
    ...
    if (((pipeName[4] != 'p') && (pipeName[4] != 'P')) ||
        ...
        !shims::is_path_separator(pipeName[8]))
    {
        return false;
    }
