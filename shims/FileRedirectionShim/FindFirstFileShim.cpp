...
struct find_deleter
{
    using pointer = shims::fancy_handle;
    ...
    _In_ DWORD additionalFlags) noexcept try
{
    ...
    WIN32_FIND_DATAW* findData = shims::is_ansi<CharT> ? &result->cached_data : wideData;
...
template <typename CharT>
HANDLE __stdcall FindFirstFileShim(_In_ const CharT* fileName, _Out_ win32_find_data_t<CharT>* findFileData) noexcept
{
    // For simplicity, just do what the OS appears to be doing and forward arguments on to FindFirstFileEx
    return FindFirstFileExShim(fileName, FindExInfoStandard, findFileData, FindExSearchNameMatch, nullptr, 0);
}
DECLARE_STRING_SHIM(impl::FindFirstFile, FindFirstFileShim);

template <typename CharT>
BOOL __stdcall FindNextFileShim(_In_ HANDLE findFile, _Out_ win32_find_data_t<CharT>* findFileData) noexcept try
{
    auto guard = g_reentrancyGuard.enter();
    if (!guard)
    {
...
DECLARE_STRING_SHIM(impl::FindNextFile, FindNextFileShim);

BOOL __stdcall FindCloseShim(_Inout_ HANDLE findHandle) noexcept
{
    auto guard = g_reentrancyGuard.enter();
    if (!guard)
    {
...
DECLARE_SHIM(impl::FindClose, FindCloseShim);
