...
void InitializePaths()
{
    ...
    auto packageRootPath = ::ShimQueryPackageRootPath();
    if (auto pathType = shims::path_type(packageRootPath);
        (pathType == shims::dos_path_type::root_local_device) || (pathType == shims::dos_path_type::local_device))
    {
        packageRootPath += 4;
    }
    assert(shims::path_type(packageRootPath) == shims::dos_path_type::drive_absolute);
    g_packageRootPath = shims::remove_trailing_path_separators(packageRootPath);
    g_packageVfsRootPath = g_packageRootPath / L"VFS";

    // Ensure that the redirected root path exists
    g_redirectRootPath = shims::known_folder(FOLDERID_LocalAppData) / L"VFS";
    ...
    g_vfsFolderMappings.push_back(vfs_folder_mapping{ shims::known_folder(FOLDERID_SystemX86), LR"(SystemX86)"sv });
...
#endif
    g_vfsFolderMappings.push_back(vfs_folder_mapping{ shims::known_folder(FOLDERID_Windows), LR"(Windows)"sv });
    g_vfsFolderMappings.push_back(vfs_folder_mapping{ shims::known_folder(FOLDERID_ProgramData), LR"(Common AppData)"sv });
    g_vfsFolderMappings.push_back(vfs_folder_mapping{ shims::known_folder(FOLDERID_System) / LR"(catroot)"sv, LR"(AppVSystem32Catroot)"sv });
    g_vfsFolderMappings.push_back(vfs_folder_mapping{ shims::known_folder(FOLDERID_System) / LR"(catroot2)"sv, LR"(AppVSystem32Catroot2)"sv });
    g_vfsFolderMappings.push_back(vfs_folder_mapping{ shims::known_folder(FOLDERID_System) / LR"(drivers\etc)"sv, LR"(AppVSystem32DriversEtc)"sv });
    g_vfsFolderMappings.push_back(vfs_folder_mapping{ shims::known_folder(FOLDERID_System) / LR"(driverstore)"sv, LR"(AppVSystem32Driverstore)"sv });
    g_vfsFolderMappings.push_back(vfs_folder_mapping{ shims::known_folder(FOLDERID_System) / LR"(logfiles)"sv, LR"(AppVSystem32Logfiles)"sv });
    g_vfsFolderMappings.push_back(vfs_folder_mapping{ shims::known_folder(FOLDERID_System) / LR"(spool)"sv, LR"(AppVSystem32Spool)"sv });
}
        ...
        if (auto pathsValue = rootObject.try_get("redirectedPaths"))
        {
            ...
            auto initializeRedirection = [](const std::filesystem::path& basePath, const shims::json_array& specs)
            {
                for (auto& spec : specs)
                {
                    ...
                    auto path = shims::remove_trailing_path_separators(basePath / specObject.get("base").as_string().wstring());
...
{
    return std::equal(basePath.native().begin(), basePath.native().end(), path, shims::path_compare{});
}
...
normalized_path NormalizePathImpl(const CharT* path)
{
    ...
    auto pathType = shims::path_type(path);
    if (pathType == shims::dos_path_type::root_local_device)
    {
        // Root-local device paths are a direct escape into the object manager, so don't normalize them
        result.full_path = widen(path);
    }
    else if (pathType != shims::dos_path_type::unknown)
    {
        result.full_path = widen(shims::full_path(path));
        pathType = shims::path_type(result.full_path.c_str());
    }
    else // unknown
    {
        return result;
    ...
    if (pathType == shims::dos_path_type::drive_absolute)
    {
        result.drive_absolute_path = result.full_path.data();
    }
    else if ((pathType == shims::dos_path_type::local_device) || (pathType == shims::dos_path_type::root_local_device))
    {
        auto candidatePath = result.full_path.data() + 4;
        if (shims::path_type(candidatePath) == shims::dos_path_type::drive_absolute)
        {
            result.drive_absolute_path = candidatePath;
        }
    }
    else if (pathType == shims::dos_path_type::unc_absolute)
    {
...
normalized_path DeVirtualizePath(normalized_path path)
{
    if (path.drive_absolute_path && path_relative_to(path.drive_absolute_path, g_packageVfsRootPath))
    {
        ...
        if (shims::is_path_separator(packageRelativePath[0]))
        {
            ++packageRelativePath;
            for (auto& mapping : g_vfsFolderMappings)
            {
                if (path_relative_to(packageRelativePath, mapping.package_vfs_relative_path))
                {
                    ...
                    if (shims::is_path_separator(vfsRelativePath[0]))
                    {
                        ++vfsRelativePath;
                    }
...
std::wstring RedirectedPath(const normalized_path& deVirtualizedPath, bool ensureDirectoryStructure)
{
    ...
    assert(shims::path_type(deVirtualizedPath.drive_absolute_path) == shims::dos_path_type::drive_absolute);
    result.push_back(L'\\');
...
static path_redirect_info ShouldRedirectImpl(const CharT* path, redirect_flags flags)
{
    ...
    for (auto& redirectSpec : g_redirectionSpecs)
    {
        if (path_relative_to(normalizedPath.drive_absolute_path, redirectSpec.base_path))
        {
            ...
            if (shims::is_path_separator(relativePath[0]))
            {
                ++relativePath;
            }
