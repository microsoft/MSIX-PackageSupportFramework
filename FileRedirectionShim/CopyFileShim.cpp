...
BOOL __stdcall CopyFileShim(_In_ const CharT* existingFileName, _In_ const CharT* newFileName, _In_ BOOL failIfExists) noexcept
{
    ...
    _In_ DWORD copyFlags) noexcept
{
    ...
    try
    {
        if (guard)
        {
            // See note in CopyFileShim for commentary on copy-on-read policy
            auto [redirectSource, sourceRedirectPath] = ShouldRedirect(existingFileName, redirect_flags::check_file_presence);
            auto [redirectDest, destRedirectPath] = ShouldRedirect(newFileName, redirect_flags::ensure_directory_structure);
            if (redirectSource || redirectDest)
            {
...
DECLARE_STRING_SHIM(impl::CopyFileEx, CopyFileExShim);
...
DECLARE_SHIM(impl::CopyFile2, CopyFile2Shim);
