    ...
    _Reserved_ LPVOID reserved) noexcept
{
    ...
    try
    {
        if (guard)
        {
            ...
            auto [redirectTarget, targetRedirectPath, shouldReadonlyTarget] = ShouldRedirect(replacedFileName, redirect_flags::copy_on_read, ReplaceFileInstance);
            auto [redirectSource, sourceRedirectPath, shouldReadonlySource] = ShouldRedirect(replacementFileName, redirect_flags::copy_on_read, ReplaceFileInstance);
            auto [redirectBackup, backupRedirectPath, shouldReadonlyDest] = ShouldRedirect(backupFileName, redirect_flags::ensure_directory_structure, ReplaceFileInstance);
