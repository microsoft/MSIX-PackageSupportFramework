    ...
    _Reserved_ LPSECURITY_ATTRIBUTES securityAttributes) noexcept
{
    ...
    try
    {
        if (guard)
        {
            ...
            auto [redirectLink, redirectPath, shoudReadonly] = ShouldRedirect(fileName, redirect_flags::ensure_directory_structure);
            auto [redirectTarget, redirectTargetPath, shoudReadonly] = ShouldRedirect(existingFileName, redirect_flags::copy_on_read);
